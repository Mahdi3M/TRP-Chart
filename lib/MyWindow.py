...
class CustomWindow(QMainWindow):
    ...
    def showAnalysisPage(self):
        ...
        for col in range(7):
            if col in [3, 4]:
                self.analysis_table.setColumnWidth(col, max(self.resolution.width()//7, 220))
            elif col in [5, 6]:
                self.analysis_table.setColumnWidth(col, max((self.resolution.width()-100)//7, 120))
            else:
                self.analysis_table.setColumnWidth(col, max((self.resolution.width()-275)//7, 100))
    ...
    def showResultPage(self):
        ...
        self.loading_gif.setMovie(self.loading)
